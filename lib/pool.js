...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        // console.log("Get block template:");
        ...
        //console.log("Previous hash check:"+JSON.stringify(previous_hash));
        if (!currentBlockTemplate || previous_hash.toString('hex') != currentBlockTemplate.previous_hash.toString('hex')){
            log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d', [, result.difficulty]);
            console.log('New block to mine at height' +result.height +  " difficulty "+ result.difficulty);
...
function processBlockTemplate(template){
    ...
    console.log("Valid block templates"+validBlockTemplates.length);
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        hash = cryptoNight(convertedBlob, convertedBlob[0]);
        shareType = 'valid';
    }
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            console.log("handleMinerMethod login");
            ...
            console.log("handleMinerMethod login checkpoint 1");
            ...
            console.log("handleMinerMethod login checkpoint 2");
            ...
            if (addressBase58Prefix !== addressPrefix && integratedAddressBase58Prefix !== addressPrefix){
                console.log("handleMinerMethod login invalid address used for login");
            ...
            break;
        case 'getjob':
            console.log("handleMinerMethod getjob");
            if (!miner){
            ...
            break;
        case 'submit':
            console.log("handleMinerMethod submit");
            if (!miner){
                sendReply('Unauthenticated');
                return;
            }
	    ...
	    params.nonce = params.nonce.substr(0, 8).toLowerCase();
            ...
            break;
        case 'keepalived' :
            console.log("handleMinerMethod keepalived");
            miner.heartbeat()
            sendReply(null, { status:'KEEPALIVED'
            });
            break;
        default:
            sendReply("handleMinerMethod invalid method");
            var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
